import React from 'react';
import { Container, Row, Col, Form, FormGroup, Label, Input, Button } from 'reactstrap';
import configData from '../config/data.json';
import landingPage from '../config/landingpage.json';

const ContactForm = ({ isRendered }) => {
  const data = configData.find(data => data.id === landingPage.id).contactFormConfig;

  if (!isRendered || !data) {
    return (null)
  }

  return (
    <section className="section" id="contact">
      <Container>
        <Row className="justify-content-center">
          <Col lg={6} md={8}>
            <div className="title text-center mb-5">
              <h3 className="font-weight-normal text-dark"><span className="text-warning">{data.title}</span></h3>
              <p className="text-muted">{data.description}</p>
            </div>
          </Col>
        </Row>
        <Row>
          <Col lg={12} md={8}>
            <Form>
              <FormGroup>
                <Label for="Name">Full name</Label>
                <Input type="input" name="fullname" id="full-name" placeholder="Full name" />
              </FormGroup>
              <FormGroup>
                <Label for="exampleEmail">Email</Label>
                <Input type="email" name="email" id="email" placeholder="Email" />
              </FormGroup>
              <FormGroup>
                <Label for="exampleEmail">Phone number</Label>
                <Input type="input" name="phone" id="phone" placeholder="Phone number" />
              </FormGroup>
              <FormGroup>
                <Label for="exampleEmail">Zip Code</Label>
                <Input type="input" name="zip" id="zip" placeholder="Zip Code" />
              </FormGroup>
              <FormGroup>
                <Label for="exampleEmail">Comment</Label>
                <Input type="textarea" name="comment" id="comment" placeholder="Comment" />
              </FormGroup>
              <Button color='primary'>{data.buttonText}</Button>
            </Form>
          </Col>
        </Row>
      </Container>
    </section>
  );
}

export default ContactForm;